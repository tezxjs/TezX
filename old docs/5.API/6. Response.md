...
This system makes it easy to return **JSON, HTML, text, files, redirects, and custom responses** with a clean API.
...
sequenceDiagram
    ...
    Handler->>Ctx: ctx.json({..}) / ctx.html(..) / ctx.text(..) / ctx.send(..)
