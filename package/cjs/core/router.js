...
class Router extends MiddlewareConfigure_1.default {
  ...
  #addRoute(method, path, callback, middlewares) {
    ...
    if (/(\/\*|\?)/.test(p)) {
      ...
      if (!config_1.GlobalConfig.overwriteMethod && handler.has(method)) return;
      ...
      for (const [segment, handlers] of router.routers) {
        ...
        if (this.routers.has(path)) {
          ...
          for (const [method, handler] of handlers) {
            if (
              !config_1.GlobalConfig.overwriteMethod &&
              baseRouter.has(method)
            )
              return;
      ...
      for (const part of parts) {
        if (part.startsWith("*")) {
          if (!rootMiddlewares.children.has("*")) {
            rootMiddlewares.children.set(
              "*",
              new MiddlewareConfigure_1.TriMiddleware(),
            );
          }
