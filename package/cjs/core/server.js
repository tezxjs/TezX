...
class TezX extends router_js_1.Router {
  ...
  async #handleRequest(req, options) {
    ...
    if (this.#onPathResolve) {
      ...
      config_js_1.GlobalConfig.debugging.warn(
        `${colors_js_1.COLORS.white} PATH RESOLVE ${colors_js_1.COLORS.reset} ${colors_js_1.COLORS.red}${pathname}${colors_js_1.COLORS.reset} ➞ ${colors_js_1.COLORS.cyan}${resolvePath}${colors_js_1.COLORS.reset}`,
      );
    }
    if (typeof resolvePath !== "string") {
      throw new Error(
        `Path resolution failed: expected a string, got ${typeof resolvePath}`,
      );
    }
    ...
    try {
      ...
      let finalResponse = () => {
        return (ctx) => {
          ...
          const statusText =
            response?.statusText ||
            context_js_1.httpStatusMap[response?.status] ||
            "";
      ...
      config_js_1.GlobalConfig.debugging.error(
        `${colors_js_1.COLORS.bgRed} ${ctx.pathname}, Method: ${ctx.method} ${colors_js_1.COLORS.reset}`,
        `${context_js_1.httpStatusMap[500]}: ${error} `,
      );
