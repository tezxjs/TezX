...
class Context {
  ...
  constructor(req, options) {
    ...
    if (
      environment_js_1.EnvironmentDetector.getEnvironment == "node" ||
      ...
      const protocol =
        typeof encrypted === "boolean"
          ? encrypted
            ...
            : "http"
          : "http";
      const host = environment_js_1.EnvironmentDetector.getHost(
        this.#requestHeaders,
      );
      const path = req.url || "/";
  ...
  async download(filePath, fileName) {
    try {
      ...
      if (runtime === "node") {
        const { existsSync } = await Promise.resolve().then(() =>
          require("fs"),
        );
      ...
      let fileBuffer;
      if (runtime === "node") {
        const { readFileSync } = await Promise.resolve().then(() =>
          require("fs"),
        );
  ...
  async sendFile(filePath, ...args) {
    try {
      ...
      const mimeType =
        staticFile_js_1.mimeTypes[ext] || staticFile_js_1.defaultMimeType;
      let fileStream;
      if (runtime === "node") {
        const { createReadStream } = await Promise.resolve().then(() =>
          require("fs"),
        );
