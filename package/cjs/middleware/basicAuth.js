  ...
  return async (ctx, next) => {
    ...
    if (!authMethod || !supportedMethods.includes(authMethod)) {
      config_js_1.GlobalConfig.debugging.error(
        `${colors_js_1.COLORS.bgRed}[AUTH]${colors_js_1.COLORS.reset} Unsupported or missing authentication method.`,
      );
    ...
    try {
      ...
      config_js_1.GlobalConfig.debugging.error(
        `${colors_js_1.COLORS.bgRed}[AUTH]${colors_js_1.COLORS.reset} Failure for method: ${ctx.method}`,
      );
