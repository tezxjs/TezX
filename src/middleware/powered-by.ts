 ...
 * ```
 */
const poweredBy = <T extends Record<string, any> = {}, Path extends string = any>(serverName?: string): Middleware<T, Path> => {
  return function poweredBy(ctx, next) {
    ctx.headers.set("x-powered-by", serverName || "TezX"); // Set the header with a default value
