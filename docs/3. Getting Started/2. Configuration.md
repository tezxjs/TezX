...
```typescript
import { loadEnv, logger, TezX } from "tezx";
...
export type TezXConfig = {
  ...
  /**
   * Optional logging function for application-wide tracking.
   * Must conform to `LoggerFnType` (defined separately).
   */
  logger?: LoggerFnType;
} & RouterConfig;
...
- **Example**:
  ...
  ```typescript
  server.get("/products", (ctx) => ctx.text("Old"));
...
- **Type**: `LoggerFnType` (custom type, assumed defined elsewhere)
- **Default**: `undefined`
- **Purpose**: Provides a custom logging function for tracking application events.
- **Behavior**: Invoked by `TezX` for logging requests, responses, or errors.
- **Example**:
...
- **Example**:
  ...
  server.get("/products", (ctx) => ctx.text("Products")); // /api/products
  ```
...
1. **Protect Routes in Production**
   ...
   - Set `overwriteMethod: false` to avoid accidental overwrites:
     ...
     ```typescript
     const server = new TezX({ overwriteMethod: false });
     ```
...
2. **Organize with `basePath`**
   ...
   - Use prefixes like `/v1` or `/api` for clarity:
     ...
     ```typescript
     const server = new TezX({ basePath: "/v1" });
     ```
...
4. **Safely Access `env`**
   ...
   - Check critical variables:
     ...
     ```typescript
     const port = server.config.env.PORT || "3000";
     ```
