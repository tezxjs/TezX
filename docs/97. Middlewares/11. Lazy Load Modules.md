...
const lazyLoadMiddleware = lazyLoadModules({
  moduleKey: (ctx: Context) => {
    ...
    return (
      ctx.req.body?.moduleName ||
      ctx.req.query.module ||
      ctx.req.params.module
    );
  },
  ...
  lifecycleHooks: {
    onLoad: (moduleName, ctx) =>
      console.log(`Loading ${moduleName} for request ID: ${ctx.req.id || "unknown"}`),
  ...
  validateModule: (module) => {
    ...
    return typeof module.handler === "function" && typeof module.version === "string";
  },
});
