...
```ts
import { rateLimiter, getConnInfo } from "tezx/middleware";
```
...
export type RateLimiterOptions = {
  ...
  // default: ctx.req.remoteAddress.address from getConnInfo()
...
app.use(
  rateLimiter({
    ...
    keyGenerator: (ctx) => ctx.user?.id || ctx.req.remoteAddress.address,
  })
);
```
...
import { rateLimiter, getConnInfo, logger, cors } from "tezx/middleware";
...
1. Use `getConnInfo()` **before** the middleware to extract client IP.
2. Configure `maxRequests` and `windowMs` for your API.
3. Optionally provide `keyGenerator`, `storage`, and `onError` for full customization.
